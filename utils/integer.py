...
def quantize_int_weight(module):
    ...
    assert (
        module.w_bit == 8
    ), f"module {module}'s weight is quantized with {module.w_bit} bits"

    w_int = (
        (module.weight / module.w_interval)
        .round_()
        .clamp_(-module.w_qmax, module.w_qmax - 1)
    )
...
def quantize_matmul_input(
    input, interval, qmax, n_G, n_V, n_H, crb_groups, crb_rows, crb_cols
):
    """
    ...
    x = x.view(-1, n_G * crb_groups, n_V * crb_rows, n_H * crb_cols)
    x = x[
        :, : x.shape[1] - pad_groups, : x.shape[2] - pad_rows, : x.shape[3] - pad_cols
    ]
...
def quantize_int_activation(module, input):
    ...
    """
    if isinstance(module, PostGeluPTQSLQuantLinear) or isinstance(
        module, PostGeluPTQSLBatchingQuantLinear
    ):
        assert (
            module.a_bit == 8
        ), f"module {module}'s activation is quantized with {module.a_bit} bits"
        ...
        int_input_neg = (
            (x / module.a_neg_interval).round_().clamp_(-module.a_qmax + 1, 0).abs()
        )
        ...
        x = input[0]
        int_input = (
            (x / module.a_interval).round_().clamp_(-module.a_qmax, module.a_qmax - 1)
        )
        int_input = int_input.cpu().detach().to(torch.int8)

        module.int_input = [int_input]

    elif isinstance(module, SoSPTQSLQuantMatMul) or isinstance(
        module, SoSPTQSLBatchingQuantMatMul
    ):
        assert (
            module.A_bit == 8
        ), f"module {module}'s matrix A is quantized with {module.A_bit} bits"
        assert (
            module.B_bit == 8
        ), f"module {module}'s matrix B is quantized with {module.B_bit} bits"

        A, B = input[0], input[1]

        A_high = (
            (A.clamp(module.split, 1) * (module.A_qmax - 1))
            .round_()
            .clamp_(0, module.A_qmax - 1)
        )
        A_high = A_high.detach().to(torch.uint8) + 128

        A_low = (
            (A.clamp(0, module.split) / module.A_interval)
            .round_()
            .clamp_(0, module.A_qmax - 1)
        )
    ...
    elif isinstance(module, PTQSLQuantMatMul) or isinstance(
