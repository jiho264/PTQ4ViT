...
class LoaderGenerator:
    ...
    def calib_loader(self, num=1024, seed=3):
        if self._calib_set is None:
            ...
            self._calib_set = torch.utils.data.Subset(
                copy.deepcopy(self.train_set), inds
            )
            self._calib_set.dataset.transform = self.test_transform
        return torch.utils.data.DataLoader(
            self._calib_set, batch_size=num, shuffle=False, **self.train_loader_kwargs
        )
...
class CIFARLoaderGenerator(LoaderGenerator):
    ...
    def train_set(self):
        if self._train_set is None:
            self._train_set = self.dataset_fn(
    ...
    def test_set(self):
        if self._test_set is None:
            self._test_set = self.dataset_fn(
                self.root, train=False, transform=self.test_transform
            )
    ...
    def train_set(self):
        if self._train_set is None:
            self._train_set = ImageFolder(
                os.path.join(self.root, "train"), self.train_transform
            )
        return self._train_set
    ...
    def test_set(self):
        if self._test_set is None:
            self._test_set = ImageFolder(
                os.path.join(self.root, "val"), self.test_transform
            )
        return self._test_set
...
class FasterImageNetLoaderGenerator(ImageNetLoaderGenerator):
    def test_loader(self, shuffle=False, batch_size=None):
        ...
        return torch.utils.data.DataLoader(
            dataset, batch_size=batch_size, shuffle=shuffle, **self.test_loader_kwargs
        )
        ...
        class DebugSet(torch.utils.data.Dataset):
            def __getitem__(self, idx):
                ...
                if version == "3":
                    ...
                    raise NotImplementedError(
                        f"version {version} of Debug dataset is not supported"
                    )
...
def get_dataset(args: argparse.Namespace):
    ...
    test_batch_size = (
        args.test_batch_size if hasattr(args, "test_batch_size") else batch_size
    )
    val_fraction = args.val_fraction if hasattr(args, "val_fraction") else 0
    if "cifar" in dataset_name:
        # Data loading code
        g = CIFARLoaderGenerator(
            dataset_root, args.dataset, batch_size, test_batch_size, num_workers
        )
    # elif "coco" in dataset_name:
    #     g = COCOLoaderGenerator(
    #         dataset_root, args.dataset, batch_size, test_batch_size, num_workers
    #     )
    elif "debug" in dataset_name:
        g = DebugLoaderGenerator(
            dataset_root, args.dataset, batch_size, test_batch_size, num_workers
        )
    elif args.dataset == "ImageNet":
        g = ImageNetLoaderGenerator(
            dataset_root, args.dataset, batch_size, test_batch_size, num_workers
        )
    else:
        raise NotImplementedError
