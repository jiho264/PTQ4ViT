...
class PTQSLQuantMatMul(MinMaxQuantMatMul):
    ...
    def _get_similarity(self, tensor_raw, tensor_sim, metric=None, dim=-1):
        ...
        if metric == "cosine":
            ...
            similarity = F.cosine_similarity(
                tensor_raw - torch.mean(tensor_raw),
                tensor_sim - torch.mean(tensor_sim),
                dim=dim,
            )
        else:
            if metric == "L1_norm":
    ...
    def _search_best_A_interval(self, A, B, A_interval_candidates):
        ...
        # recalculate A_pad
        A_pad = (
            F.pad(A, [0, self.pad_cols_A, 0, self.pad_rows_A, 0, self.pad_groups_A])
            .unsqueeze(0)
            .view(
                1,
                -1,
        ...
        for v, h in product(range(self.n_V_A), range(self.n_H_A)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                similarity = self._get_similarity(
                    self.raw_out, out_sim, self.metric
                )  # shape: parallel_eq_n,B,H,dim1
                similarity = similarity.mean(
                    [1, 3]
                )  # shape: parallel_eq_n,H (remaining mean operation will be done later on)
                similarities.append(similarity)
            # calculate best similarity for this block
            similarities = torch.cat(similarities, 0)  # shape: eq_n,H
            similarities = (
                F.pad(similarities, [0, self.pad_groups_A])
                .view(self.eq_n, self.n_G_A, self.crb_groups_A)
                .mean(-1)
            )  # shape: eq_n, n_G_A
            best_index = torch.argmax(similarities, dim=0, keepdim=False).view(
                1, 1, -1, 1, 1, 1, 1, 1
            )
    ...
    def _search_best_B_interval(self, A, B, B_interval_candidates):
        ...
        # recalculate B_pad
        B_pad = (
            F.pad(B, [0, self.pad_cols_B, 0, self.pad_rows_B, 0, self.pad_groups_B])
            .unsqueeze(0)
            .view(
                1,
                -1,
        ...
        for v, h in product(range(self.n_V_B), range(self.n_H_B)):
            ...
            similarities = torch.cat(similarities, 0)  # shape: eq_n,H
            similarities = (
                F.pad(similarities, [0, self.pad_groups_B])
    ...
    def _initialize_intervals(self, A, B):
        ...
        )  # shape: 1,B,n_G,crb_groups,n_V,crb_rows,n_H,crb_cols
        B_pad = (
            F.pad(B, [0, self.pad_cols_B, 0, self.pad_rows_B, 0, self.pad_groups_B])
            .unsqueeze(0)
            .view(
        ...
        if self.init_layerwise:
            ...
            self.A_interval = (
                (A_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.A_qmax - 0.5))
                .detach()
                .squeeze(0)
            )  # shape: 1,n_G,1,n_V,1,n_H,1
            self.B_interval = (
                (B_pad.abs().amax([0, 1, 3, 5, 7], keepdim=True) / (self.B_qmax - 0.5))
                .detach()
                .squeeze(0)
            )  # shape: 1,n_G,1,n_V,1,n_H,1
...
class SoSPTQSLQuantMatMul(PTQSLQuantMatMul):
    ...
    def _search_best_A_interval(self, A, B, split_candidates):
        ...
        for i in range(len(split_candidates)):
            ...
            A_high = (
                A_.clamp(split_candidates[i], 1) * (self.A_qmax - 1)
            ).round_().clamp_(0, self.A_qmax - 1) / (self.A_qmax - 1)
            A_low = (A_.clamp(0, split_candidates[i]) / cur_A_interval).round_().clamp_(
                0, self.A_qmax - 1
            ) * cur_A_interval
