...
class QuantileQuantConv2d(MinMaxQuantConv2d):
    ...
    def _quantile(self, tensor, quantile):
        if tensor.numel() >= 16777216:
            ...
            return torch.quantile(
                tensor.view(-1)[: 16777216 * n].view(n, 16777216), quantile, 1
            ).mean()
        else:
    ...
    def calibration_step2(self, x):
        ...
        self.w_interval = (
            self._quantile(self.weight.data.abs(), self.w_quantile)
            / (self.w_qmax - 0.5)
        ).detach()
        self.a_interval = (
            self._quantile(x.abs(), self.a_quantile) / (self.a_qmax - 0.5)
        ).detach()
        self.calibrated = True
        out = self.quant_forward(x)
        return out
...
class PTQSLQuantConv2d(MinMaxQuantConv2d):
    ...
    def quant_weight_bias(self):
        ...
        w_sim = self.weight.view(
            self.n_V, oc // self.n_V, self.n_H, (ic * kw * kh) // self.n_H
        )
        w_sim = (
            (w_sim / self.w_interval)
            .round_()
            .clamp(-self.w_qmax, self.w_qmax - 1)
            .mul_(self.w_interval)
        )
    ...
    def _search_best_w_interval(self, x, weight_interval_candidates):
        ...
        for v, h in product(range(self.n_V), range(self.n_H)):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                cur_w_interval[:, v : v + 1, :, h : h + 1, :] = (
                    weight_interval_candidates[p_st:p_ed, v : v + 1, :, h : h + 1, :]
                )
                # quantize weight and bias
                oc, ic, kw, kh = self.weight.data.shape
                w_sim = self.weight.view(
                    self.n_V, oc // self.n_V, self.n_H, -1
                ).unsqueeze(
                    0
                ...
                similarity = self._get_similarity(
                    self.raw_out, out_sim, self.metric, dim=2
                )  # shape: B,parallel_eq_n,fw,fh
            ...
            tmp_w_interval[:, v : v + 1, :, h : h + 1, :] = torch.gather(
                weight_interval_candidates[:, v : v + 1, :, h : h + 1, :],
                dim=0,
                index=best_index,
            )
    ...
    def _search_best_a_interval(self, x, input_interval_candidates):
        ...
        for p_st in range(0, self.eq_n, self.parallel_eq_n):
            ...
            similarity = self._get_similarity(
                self.raw_out.transpose(0, 1), out_sim, self.metric, dim=2
            )  # shape: parallel_eq_n,B,fw,fh
    ...
    def _initialize_intervals(self, x):
        ...
        if self.init_layerwise:
            ...
            self.w_interval = self.weight.view(
                self.n_V, self.out_channels // self.n_V, self.n_H, -1
...
class BatchingEasyQuantConv2d(PTQSLQuantConv2d):
    ...
    def _get_similarity(
        ...
        if metric == "cosine":
            ...
            similarity = F.cosine_similarity(
                tensor_raw.view(b, 1, -1),
                tensor_sim.view(b, parallel_eq_n, -1),
                dim=dim,
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st + self.parallel_eq_n)
                cur_w_interval = weight_interval_candidates[
                    p_st:p_ed
                ...
                similarity = self._get_similarity(
                    raw_out, out_sim, self.metric, dim=-3, raw_grad=raw_grad
                )  # shape: b,parallel_eq_n,fw,fh
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                p_ed = min(self.eq_n, p_st + self.parallel_eq_n)
                cur_a_interval = input_interval_candidates[
                    p_st:p_ed
                ]  # shape: parallel_eq_n,1,1,1,1
                # quantize weight and bias
                w_sim, bias_sim = self.quant_weight_bias()
                ...
                similarity = torch.mean(
                    similarity, dim=[3, 4]
                )  # shape: parallel_eq_n,b
                similarity = torch.sum(
                    similarity, dim=1, keepdim=True
                )  # shape: parallel_eq_n,1
                similarities.append(similarity)
