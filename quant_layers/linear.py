...
class PTQSLQuantLinear(MinMaxQuantLinear):
    ...
    def quant_weight_bias(self):
        ...
        w = (
            (
                self.weight.view(self.n_V, self.crb_rows, self.n_H, self.crb_cols)
                / self.w_interval
            )
            .round_()
            .clamp_(-self.w_qmax, self.w_qmax - 1)
    ...
    def quant_input(self, x):
        ...
        x_sim = (
            (x_sim.div_(self.a_interval)).round_().clamp_(-self.a_qmax, self.a_qmax - 1)
        )
        x_sim = x_sim.mul_(self.a_interval).reshape_as(x)
        return x_sim

    def _search_best_w_interval(
        self, x, weight_interval_candidates, raw_out_expanded_chunked
    ):
        """
        ...
        for h in range(self.n_H):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                w_sim = self.weight.view(
                    ...
                    0
                )  # shape: 1,n_V,crb_rows,n_H,crb_cols
                w_sim = (
                    (w_sim / cur_w_interval)
                    .round_()
                    .clamp_(-self.w_qmax, self.w_qmax - 1)
                    .mul_(cur_w_interval)
                )  # shape: parallel_eq_n,n_V,crb_rows,n_H,crb_cols
                ...
                out_sim = F.linear(
                    x_sim, w_sim, bias_sim
                )  # shape: B,*,parallel_eq_n*oc
                out_sim = torch.cat(
                    torch.chunk(out_sim.unsqueeze(-2), chunks=p_ed - p_st, dim=-1),
                    dim=-2,
                )  # shape: B,*,parallel_eq_n,oc
                out_sim = torch.cat(
                    torch.chunk(out_sim.unsqueeze(-2), chunks=self.n_V, dim=-1), dim=-2
                )  # shape: B,*,parallel_eq_n,n_V,crb_rows
                similarity = self._get_similarity(
                    raw_out_expanded_chunked, out_sim, self.metric
                )  # shape: B,*,parallel_eq_n,n_V
                similarity = torch.mean(
                    similarity, dim=list(range(len(similarity.shape) - 2))
                )  # shape: parallel_eq_n, n_V
                similarities.append(similarity)
            # store best weight interval of h into tmp_w_interval
            similarities = torch.cat(similarities, dim=0)  # shape: eq_n, n_V
            h_best_index = similarities.argmax(dim=0).reshape(
                1, -1, 1, 1, 1
            )  # shape: 1,n_V,1,1,1
        ...
        for a in range(self.n_a):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                cur_a_interval = tmp_a_interval.repeat(
                    1, 1, p_ed - p_st
                )  # shape: n_a,1,parallel_eq_n
                cur_a_interval[a : a + 1, :, :] = input_interval_candidates[
                    a : a + 1, :, p_st:p_ed
                ]
                # quantize weight and bias
                w_sim, bias_sim = self.quant_weight_bias()
                # quantize input
                x_sim = torch.cat(
                    torch.chunk(x.unsqueeze(-2), chunks=self.n_a, dim=-1), dim=-2
                ).unsqueeze(-1)
                x_sim = (x_sim / (cur_a_interval)).round_().clamp_(
                    -self.a_qmax, self.a_qmax - 1
                ) * (
                    cur_a_interval
                )  # shape: B,*,n_a,crb_acts,parallel_eq_n
                x_sim = x_sim.permute(
                    *list(range(len(x_sim.shape) - 3)), -1, -3, -2
                ).reshape(
                    *x.shape[:-1], p_ed - p_st, x.shape[-1]
                )  # shape: B,*,parallel_eq_n,ic
                # calculate similarity and store them
                out_sim = F.linear(
                    x_sim, w_sim, bias_sim
                )  # shape: B,*,parallel_eq_n,oc
                similarity = self._get_similarity(
                    raw_out_expanded, out_sim, self.metric
                )  # shape: B,*,parallel_eq_n
                similarity = torch.mean(
                    similarity, dim=list(range(len(similarity.shape) - 1))
                )  # shape: parallel_eq_n
                similarities.append(similarity)
            ...
            tmp_a_interval[a : a + 1, :, :] = torch.gather(
                input_interval_candidates[a : a + 1, :, :], dim=2, index=a_best_index
            )
    ...
    def _initialize_intervals(self, x):
        if self.init_layerwise:
            self.w_interval = (
                ...
                .view(1, 1, 1, 1)
                .repeat(self.n_V, 1, self.n_H, 1)
            )
            self.a_interval = (
                (x.abs().max() / (self.a_qmax - 0.5))
                .detach()
                .view(1, 1)
                .repeat(self.n_a, 1)
            )
        else:
            self.w_interval = self.weight.view(
                self.n_V, self.crb_rows, self.n_H, self.crb_cols
            ).abs().amax([1, 3], keepdim=True) / (self.w_qmax - 0.5)
            self.a_interval = (
                (
                    x.view(*x.shape[:-1], self.n_a, self.crb_acts)
                    .abs()
                    .amax(list(range(len(x.shape) - 1)) + [-1], keepdim=False)
                )
                / (self.a_qmax - 0.5)
            ).unsqueeze(-1)
    ...
    def calibration_step2(self, x):
        ...
        weight_interval_candidates = torch.tensor(
            ...
            0
        )  # shape: eq_n,n_V,1,n_H,1
        input_interval_candidates = torch.tensor(
            [
                self.eq_alpha + i * (self.eq_beta - self.eq_alpha) / self.eq_n
                for i in range(self.eq_n + 1)
            ]
        ).cuda().view(1, 1, -1) * self.a_interval.unsqueeze(
            -1
        )  # shape: n_a,1,eq_n
        for e in range(self.search_round):
            # search for best weight interval
            self._search_best_w_interval(
                x, weight_interval_candidates, raw_out_expanded_chunked
            )
...
class PostGeluPTQSLQuantLinear(PTQSLQuantLinear):
    ...
    def quant_input(self, x):
        ...
        x_pos = (
            (x_ / (self.a_interval[0]))
            .round_()
            .clamp_(0, self.a_qmax - 1)
            .mul_(self.a_interval[0])
        )
        x_neg = (
            (x_ / (self.a_interval[1]))
            .round_()
            .clamp_(-self.a_qmax, 0)
            .mul_(self.a_interval[1])
        )
    ...
    def _search_best_a_interval(self, x, input_interval_candidates, raw_out_expanded):
        ...
        for a in range(self.n_a):
            ...
            for p_st in range(0, self.eq_n, self.parallel_eq_n):
                ...
                x_pos = (x_sim / (cur_a_interval)).round_().clamp_(
                    0, self.a_qmax - 1
                ) * (
                    cur_a_interval
                )  # shape: B,*,n_a,crb_acts,parallel_eq_n
                x_neg = (x_sim / (self.a_interval[1])).round_().clamp_(
                    -self.a_qmax, 0
                ) * (
                    self.a_interval[1]
                )  # shape: B,*,n_a,crb_acts,1
                x_sim = (
                    (x_pos + x_neg)
                    .permute(*list(range(len(x_sim.shape) - 3)), -1, -3, -2)
                    .reshape(*x.shape[:-1], p_ed - p_st, x.shape[-1])
                )  # shape: B,*,parallel_eq_n,ic
                # calculate similarity and store them
                out_sim = F.linear(
                    x_sim, w_sim, bias_sim
    ...
    def _initialize_intervals(self, x):
        if self.init_layerwise:
            ...
            self.a_interval = [
                (x.max() / (self.a_qmax - 0.5)).detach().view(1, 1).repeat(self.n_a, 1)
            ]
        else:
            self.w_interval = self.weight.view(
                self.n_V, self.crb_rows, self.n_H, self.crb_cols
            ...
            self.a_interval = [
                (
                    (
                        x.view(*x.shape[:-1], self.n_a, self.crb_acts).amax(
                            list(range(len(x.shape) - 1)) + [-1], keepdim=False
                        )
                    )
                    / (self.a_qmax - 0.5)
                ).unsqueeze(-1)
    ...
    def calibration_step2(self, x):
        ...
        ).cuda().view(1, 1, -1) * self.a_interval[0].unsqueeze(
            -1
        )  # shape: n_a,1,eq_n
        for e in range(self.search_round):
            # search for best weight interval
...
class PTQSLBatchingQuantLinear(PTQSLQuantLinear):
    ...
    def _initialize_intervals(self):
        ...
        for b_st in range(0, self.calib_size, self.calib_batch_size):
            ...
            if self.init_layerwise:
                ...
                a_interval_ = (
                    (
                        x_.view(*x_.shape[:-1], self.n_a, self.crb_acts)
                        .abs()
                        .amax(list(range(len(x_.shape) - 1)) + [-1], keepdim=False)
                    )
                    / (self.a_qmax - 0.5)
                ).unsqueeze(-1)
    ...
    def _get_pearson_w(self, tensor_raw, tensor_sim):
        ...
        tensor_sim = (
            tensor_sim.transpose(-1, -3).contiguous_().view(b, -1, n_V, parallel_eq_n)
        )
        ...
        similarity = torch.cosine_similarity(
            tensor_raw - tensor_raw_mean, tensor_sim - tensor_sim_mean, dim=1
        )  # shape: b,n_V,parallel_eq_n
    ...
    def _get_pearson_a(self, tensor_raw, tensor_sim):
        ...
        tensor_sim = (
            tensor_sim.transpose(-1, -2).contiguous_().view(b, -1, parallel_eq_n)
        )
    ...
    def _search_best_w_interval(self, weight_interval_candidates):
        ...
        for h in range(self.n_H):
            batch_similarities = (
                []
            )  # similarities, need to concatenate and calculate sum (equivalent to mean with argmax)
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                b_ed = min(self.calib_size, b_st + self.calib_batch_size)
                x = self.raw_input[b_st:b_ed].cuda()
                raw_out_expanded = (
                    self.raw_out[b_st:b_ed].cuda().unsqueeze(-2)
                )  # shape: b,*,1,oc
                raw_out_expanded = torch.cat(
                    torch.chunk(
                        raw_out_expanded.unsqueeze(-2), chunks=self.n_V, dim=-1
                    ),
                    dim=-2,
                )  # shape: b,*,1,n_V,crb_rows
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    if self.metric != "pearson":
                        ...
                        if len(similarity.shape) > 3:
                            similarity = torch.mean(
                                similarity,
                                dim=list(range(1, len(similarity.shape) - 2)),
                            )  # shape: b, parallel_eq_n, n_V
                    else:
                        similarity = self._get_pearson_w(raw_out_expanded, out_sim)
                    similarity = similarity.sum(
                        dim=0, keepdim=True
                    )  # shape: 1, parallel_eq_n, n_V
                    similarities.append(similarity)
                # store best weight interval of h into tmp_w_interval
            ...
            h_best_index = batch_similarities.argmax(dim=0).reshape(
                1, -1, 1, 1, 1
            )  # shape: 1,n_V,1,1,1
            tmp_w_interval[:, :, :, h : h + 1, :] = torch.gather(
                weight_interval_candidates[:, :, :, h : h + 1, :],
    ...
    def _search_best_a_interval(self, input_interval_candidates):
        ...
        for a in range(self.n_a):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    if self.metric != "pearson":
                        ...
                        if len(similarity.shape) > 2:
                            similarity = torch.mean(
                                similarity,
                                dim=list(range(1, len(similarity.shape) - 1)),
                            )  # shape: b, parallel_eq_n
                    else:
                        similarity = self._get_pearson_a(raw_out_expanded, out_sim)
                    similarity = torch.sum(
                        similarity, dim=0, keepdim=True
            ...
            a_best_index = batch_similarities.argmax(dim=0, keepdim=True).reshape(
                1, 1, -1
            )
            tmp_a_interval[a : a + 1, :, :] = torch.gather(
                input_interval_candidates[a : a + 1, :, :], dim=2, index=a_best_index
...
class PostGeluPTQSLBatchingQuantLinear(PTQSLBatchingQuantLinear):
    ...
    def _initialize_intervals(self):
        ...
        if self.init_layerwise:
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                x_ = self.raw_input[b_st:b_ed].cuda()
                a_interval_ = (
                    (x_.max() / (self.a_qmax - 0.5))
                    .detach()
                    .view(1, 1)
                    .repeat(self.n_a, 1)
                )
                tmp_a_intervals.append(a_interval_)
            self.a_interval = torch.cat(tmp_a_intervals, dim=1).amax(
                dim=1, keepdim=True
            )
        else:
            tmp_a_intervals = []
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                a_interval_ = (
                    (
                        x_.view(*x_.shape[:-1], self.n_a, self.crb_acts).amax(
                            list(range(len(x_.shape) - 1)) + [-1], keepdim=False
                        )
                    )
                    / (self.a_qmax - 0.5)
                ).unsqueeze(-1)
    ...
    def quant_input(self, x):
        ...
        x_neg = (
            (x_ / (self.a_neg_interval))
            .round_()
            .clamp_(-self.a_qmax, 0)
            .mul_(self.a_neg_interval)
        )
        return (x_pos + x_neg).reshape_as(x)
        ...
        for a in range(self.n_a):
            ...
            for b_st in range(0, self.calib_size, self.calib_batch_size):
                ...
                for p_st in range(0, self.eq_n, self.parallel_eq_n):
                    ...
                    x_neg = (x_sim / (self.a_neg_interval)).round_().clamp_(
                        -self.a_qmax, 0
                    ) * (
                        self.a_neg_interval
                    )  # shape: b,*,n_a,crb_acts,1
                    ...
                    similarity = torch.mean(
                        similarity, dim=list(range(1, len(similarity.shape) - 1))
                    )  # shape: b, parallel_eq_n
                    similarity = torch.sum(
                        similarity, dim=0, keepdim=True
                    )  # shape: 1, parallel_eq_n
